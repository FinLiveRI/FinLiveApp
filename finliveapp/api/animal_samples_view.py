...
from finliveapp.models import Organization, BloodSample, GasMeasurement
...
class GasSystemsView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            events = GasMeasurement.objects.create_measurements(data, organizationid, farmid)
    ...
    def get(self, request, *args, **kwargs):
        organizationid = self.request.META.get('HTTP_X_ORG', None)
        organization = get_object_or_404(Organization, id=organizationid)
        gas = GasMeasurement.objects.filter(organization=organization)
